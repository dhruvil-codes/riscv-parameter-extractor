...
The developed solution ensures that fixed ISA constants are not extracted as configurable parameters, every extracted item is grounded directly in the provided source snippet, and hallucinations are systematically audited and removed.
...
- **Context Window**: 128,000 tokens (official OpenAI specification for `gpt-4o-mini`)
- **Sampling Temperature**: `0.0` (Deterministic generation for maximum precision, consistency, and reproducibility)
...
- **Selection Rationale**: `gpt-4o-mini` offers high inference speed, strict compliance with structured JSON/Pydantic schemas, cost efficiency, and strong zero-shot instruction adherence for structured extraction tasks.

---

## 3. Approach

The project uses a clean, two-stage extraction and verification pipeline designed for high accuracy and hallucination control without complex multi-agent frameworks or heavy abstractions.
...
**Identified Limitation**: A keyword-driven approach generates false positives. Technical specifications frequently contain numeric constants, bit allocations, and descriptive architectural rules (e.g., *"sets aside a 12-bit encoding space for up to 4,096 CSRs"*). Under a simple keyword prompt, the LLM often misclassifies invariant specification rules as configurable parameters.
...
To eliminate remaining hallucinations and over-inferences, a second LLM verification stage (`VERIFIER_PROMPT`) was introduced. The verifier audits each candidate against four criteria:
...
**Systemic Safeguards**:
- **Deterministic Generation**: Fixed temperature of `0.0`.
...
Below is the exact `results.yaml` output generated by executing `main.py` against the benchmark snippets:
  ...
  - source: Custom Test Spec 3.1
    parameters:
      - name: vector_register_length
        description:
          The length of the vector registers, which is defined by the implementation
          and must be a power of two within the specified range.
...
- **Privileged Spec 2.1**: The pipeline returned an empty list (`parameters: []`). Although the text contains numeric values and architectural details (e.g., *12-bit encoding space*, *4,096 CSRs*, *top two bits*), these represent fixed ISA rules rather than implementation-variable choices. Returning an empty list demonstrates that the prompt design and verification pass successfully prevent numeric facts from being misclassified as parameters.
- **Custom Test Spec 3.1**: The pipeline dynamically extracted `vector_register_length` as an `implementation-defined` parameter and correctly isolated both power-of-two and bit-width range constraints.
